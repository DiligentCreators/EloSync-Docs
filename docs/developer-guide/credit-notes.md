...
- `apply()` locks the linked invoice with `CustomerInvoice::query()->lockForUpdate()` (deliberately **not** `withTrashed()` — the target invoice must still be active) and validates before writing anything, throwing on `status`: invoice must exist, its status must be `sent`/`partial`, and the credit note's `total` must not exceed the invoice's `balance_due` (0.01 float tolerance). It then adds `total` to `CustomerInvoice::amount_credited` and calls `CustomerInvoice::recalculateBalanceFromAmounts()` — this recomputes `balance_due` **and can advance the invoice `status`** (`sent → partial|paid`), the same as Payments posting. It also records a `credited` activity on the invoice itself.
