...
- Status machine lives on `CustomerCreditNoteStatusEnum::allowedTransitions()` / `canTransitionTo()`: `draft → issued`, `draft → void`, `issued → applied`, `issued → void`; `applied` and `void` are terminal.
- `CustomerCreditNoteService::transitionStatus()` throws `ValidationException` (422, `status` field) for disallowed transitions; used by `issue()`, `apply()`, and `void()`.
- `issue()` backfills `issue_date` to today if it wasn't already set.
- `apply()` adds the credit note's `total` to the linked `CustomerInvoice::amount_credited`, then calls `CustomerInvoice::recalculateBalanceFromAmounts()` (recomputes `balance_due` only — unlike Payments, applying a credit note does **not** drive the invoice `status`). It also records a `credited` activity on the invoice itself.
...
- Auto-numbering: `CustomerCreditNoteService::nextNumber()` reads the `credit_notes_number_prefix` tenant setting (default `CN-`), then zero-pads a running count to 5 digits — same pattern as Invoices/Payments. Not yet exposed in the Tenant Settings UI.
...
Catalog: slug `credit-notes`, category `billing`, `is_default_included = false`, `is_billable = false`, `sort_order = 30`. Registered via `DefaultModuleRegistrar` migration (migrate-only), with a follow-up migration inserting the `module_dependencies` row on `invoices`.
...
| Detail sheet | Overview, linked invoice, contact/company, line items, notes, timeline — actions: assign, add note, issue, apply, void, edit (draft only), delete |
