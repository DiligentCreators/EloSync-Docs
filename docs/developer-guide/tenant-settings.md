...
Invoice PDF company/payment keys (`invoices` group, workspace-only — no Central fallback): `company_tagline`, `company_address`, `company_phone`, `company_website`, `company_ntn`, `company_strn`, `invoice_payment_terms`, `invoice_bank_name`, `invoice_bank_account_name`, `invoice_bank_account_number`, `invoice_bank_iban`, `invoice_bank_swift`, `invoice_default_notes`. Edited in SPA **Settings → Branding** (with logo / `button_color` / `support_email`). `CustomerInvoicePdfService` embeds the tenant logo as a data URI (Dompdf remote URLs disabled) and includes these keys in the PDF cache fingerprint.
