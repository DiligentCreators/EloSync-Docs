...
`trash.retention_days` is an integer under the `general` group. Allowed product values: **30**, **90**, **365**, or **0** (forever). Default when unset is **0** (keep SoftDeletes rows until manual force-delete). The scheduled command `trash:purge-expired` force-deletes soft-deleted rows older than the window for models in `App\Support\TrashPurgeRegistry` (CRM/module SoftDeletes — excludes Users, Team Chat, and IMAP `EmailMessage`). Resolve via `TenantSettingService::get('trash.retention_days', 0, $tenant)`. Always schedule the command; it no-ops when the setting is `0`.
