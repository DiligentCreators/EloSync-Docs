...
- `post()` iterates the payment's `allocations`, adding each `amount` to its `CustomerInvoice::amount_paid`, then calls `CustomerInvoice::recalculateBalanceFromAmounts()` per invoice (recomputes `balance_due` and advances `sent → partial|paid`; a `draft` or `void` invoice is left untouched by design). `void()` does the mirror-image subtraction and recalculation.
...
- Auto-numbering: `CustomerPaymentService::nextNumber()` reads the `payments_number_prefix` tenant setting (default `PAY-`), then zero-pads a running count to 5 digits — same pattern as Invoices' `invoices_number_prefix`. Neither prefix setting is exposed in the Tenant Settings UI yet.
