...
8. **Bulk** — `POST /email/messages/bulk` with up to 25 `message_uuids` and `action` (`delete` | `move` | `flags` | `labels`). Reuses single-message services (IMAP before local DB for flags); label mode is `add` / `remove` (not replace-all). Returns `{ processed, failed[] }` with sanitized `failed[].message` (no raw IMAP text); HTTP 422 when none succeed. Permissions: `email.delete` for delete, `email.update` otherwise. Owned messages only.
