...
- PDF: `GET …/pdf` (`invoices.view`, assignee-scoped, `throttle:invoices-pdf`) renders a Dompdf document from `resources/views/invoices/pdf.blade.php` on the fly (no stored `pdf_path`). Cached (base64) by id + `updated_at` so database/Redis JSON stores stay valid UTF-8. `send()` dispatches `WarmCustomerInvoicePdfJob` on the default queue.
