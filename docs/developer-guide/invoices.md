...
- Status machine lives on `CustomerInvoiceStatusEnum::allowedTransitions()` / `canTransitionTo()`: `draft → sent|void`, `sent → void|partial|paid`, `partial → paid|void`, `paid`/`void` are terminal. `partial`/`paid` are set programmatically by [Payments](/developer-guide/payments) posting/voiding via `CustomerInvoice::applyBalanceStatus()` / `recalculateBalanceFromAmounts()` — this API only exposes user-driven `send` / `void` / `status`.
...
- Auto-numbering: `CustomerInvoiceService::nextNumber()` reads the `invoices_number_prefix` tenant setting (default `INV-`), then zero-pads a running count (`CustomerInvoice::withTrashed()->count() + 1`) to 5 digits.
