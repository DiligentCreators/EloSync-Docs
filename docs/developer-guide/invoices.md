...
- **`Unpaid → Cancelled` requires a zero ledger** — `CustomerInvoiceService::void()` is the enforcement point (not just the enum): it throws `ValidationException` (422, `status`, naming the invoice number) if `amount_paid > 0` (void the payments first) or `amount_credited > 0` (refund applied credit notes first). `CustomerInvoice::isVoidable()` mirrors this (Draft/Unpaid only, both amounts zero). `changeStatus()` (used by `POST …/status`) routes a `cancelled` target through `void()` rather than calling `transitionStatus()` directly, so both entry points share the same ledger guard.
