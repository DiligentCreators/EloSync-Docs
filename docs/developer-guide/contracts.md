...
- **Customer e-signature:** `POST …/acceptance-link` (`contracts.send`) rotates token hash + expiry. Public `GET/POST /public/contracts/accept/{token}` (throttle `contract-acceptance`) shows summary (including `acceptance_requirements` from tenant settings) / accepts with signer name+email+IP → `active`. Optional requirements (defaults off): phone, signature image (drawn|uploaded), ID document — settings `contracts.acceptance_require_*`. Enabling signature/ID settings requires Storage entitlement (API 422 + SPA switches disabled without it). Evidence columns: `accepted_by_phone`, `acceptance_user_agent`, `acceptance_signature_path` / `_method`, `acceptance_id_document_path` / `_type` / `_original_name` (paths hidden on API). Staff downloads: `GET …/acceptance-signature`, `GET …/acceptance-id-document` (timeline `acceptance_evidence_downloaded`). Orphan uploads cleaned if accept txn fails after store. Soft delete keeps files; `forceDelete` purges signature/ID objects. Resource exposes `has_acceptance_signature`, `has_acceptance_id_document`, phone/IP/method/type (not raw paths). Activity types `signature_requested` / `signed` / `acceptance_evidence_downloaded`. SPA guest page `/#/accept/contracts/:token`. Link expiry = earlier of contract `end_date` end-of-day or 30 days. Expire/terminate/internal activate clear unused links.
...
- PDF: `GET …/pdf` (`contracts.view`, assignee-scoped, `throttle:contracts-pdf`) renders from `resources/views/contracts/pdf.blade.php` via `ContractPdfService` — branded layout aligned with other sales documents; embeds customer acceptance block + signature image data URI when present (ID files are download-only).
...
Catalog: slug `contracts`, category `sales`, `is_default_included = false`, `is_billable = false`, `sort_order = 60`, version **1.7.0**. Registered via `DefaultModuleRegistrar` migration (migrate-only). 1.1.0 added opportunity auto-fill + HTML memos; 1.2.0 adds create-invoice (soft Invoices entitlement); 1.2.1 requires `acknowledge_repeat_billing` for second+ bills; 1.3.0 related inline creates; **1.5.0 PDF + customer e-signature accept links** (+ `contracts.send` / `contracts.accept`); **1.6.0 renewal reminders** via `crm:send-due-notifications` + setting `contract_renewal_notice_days`; **1.7.0 optional phone / signature image / ID document** on public accept (tenant toggles). Production readiness: [Sales document convert](/deployment/sales-document-convert-production-readiness).
...
- Domain `contract_activities` timeline (`emailed`, `signature_requested`, `signed`, `acceptance_evidence_downloaded`, plus existing types)
