...
| **Require phone on contract accept** | When on, public contract sign links require a mobile number (`contracts.acceptance_require_phone`, default **off**). Requires Contracts. |
| **Require signature image on contract accept** | When on, customers must draw or upload a signature image (`contracts.acceptance_require_signature_image`, default **off**). Requires Contracts; enabling requires Storage (Settings switch disabled / API 422 without it). |
| **Require ID document on contract accept** | When on, customers must upload a national ID or passport as proof (`contracts.acceptance_require_id_document`, default **off**). Requires Contracts; enabling requires Storage (Settings switch disabled / API 422 without it). |
...
| **Outbound webhooks** | Subscribe to domain events (`lead.created`, `customer_payment.posted`, Help Desk events, …). EloSync POSTs a signed JSON envelope to your HTTPS URL. Signing secret shown once. Use **Edit** (name / URL / events), **Send test**, **Enable/Disable**, and review **Recent deliveries**. Rotate/delete ask for confirmation. |
