...
- Payment fields: amount, currency, method (`cash`, `bank_transfer`, `cheque`, `card_manual`, `other`), paid-at date, reference, notes, optional contact/company link
...
- Status workflow: `draft → posted → void` (`POST /payments/{id}/post`, `.../void`) — posting a payment adds each allocation to its invoice's `amount_paid` and recalculates the invoice balance/status (`sent`/`partial`/`paid`); voiding a posted payment reverses those allocations
...
- Module licensing (`module:payments`) + Spatie permissions — **free Marketplace opt-in** (Billing category), **requires Invoices**
...
Enable Payments from Marketplace (free) — **Invoices must already be installed**; Marketplace blocks the install otherwise. Catalog: slug `payments`, category `billing`, `is_default_included = false`, `is_billable = false`, `sort_order = 20`.

## Related modules

**Required:** [Invoices](/user-guide/invoices-overview) — Payments cannot be installed or record allocations without it. **Optional:** Contacts and Companies (customer pickers only appear/validate when entitled). See [Module Dependencies](/architecture/module-dependencies).
