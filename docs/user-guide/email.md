...
- Use the **reading pane** control (layout icon next to search) to switch between **Right panel** (default, resizable list width) and **Full panel** (message fills the content area; **Back to list** returns to the split view). Your choice is remembered in the browser.
- Search filters the current folder (or label) list.
- Use checkboxes to **select multiple messages** (or select all in the current list), then **Mark read / unread**, **Move to**, **Labels**, or **Delete** from the bulk toolbar. Delete follows the same Trash vs permanent rules as a single message.
