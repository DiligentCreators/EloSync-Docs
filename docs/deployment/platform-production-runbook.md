...
| Object storage | `FILESYSTEM_DISK=s3` + `AWS_*` configured; migrate local assets with `php artisan storage:migrate-to-s3` ([object-storage.md](/developer-guide/object-storage)) |
...
- `invoices:generate-recurring` (daily, `withoutOverlapping(120)`, `onOneServer` — draft occurrences for due recurring customer invoices; no-op when Invoices is not entitled)
...
| Object storage (branding) | Daily | Sync from backup bucket; see [object-storage.md](/developer-guide/object-storage) |
