...
- Platform audit events: `todo_created`, `todo_updated`, `todo_deleted`
- No due digests or assignment notifications in v1

## Deploy checklist

1. Migrate `todos` table
2. Run register + permissions data migrations
3. Deploy frontend (board/list, creator-gated edit/delete)
4. Confirm `module:todos` + `todos.*` permissions
5. Smoke: register/login → ToDos board → create → list → delete own item; second user cannot see first user’s to-do; owner can view but not delete others’
